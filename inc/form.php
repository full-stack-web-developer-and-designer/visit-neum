...
?>
<form  spellcheck="false" autocomplete="off" id='contact_me' class='form ajax' name='contact_me' action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>' method='POST' autocomplete='on'>
<p id="response" class="success"><!-- This will hold response from the server --></p>
      <fieldset>
            <legend>Vaši podaci</legend>
					...
					<div class="form-control InputIconBg"><input type="text" class="input mail" name="email" id="email" placeholder="Vaš e-mail..." value="<?php echo Input::get('email'); ?>"><i class="fas fa-envelope contact_icon" aria-hidden="true"></i><span class="error"><?=$email_error;?></span></div><!-- end .form-control -->
